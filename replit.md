...
**Dynamic Product Integration and Dashboard Display**: Completed full integration of dynamic product management with real-time dashboard synchronization. Order forms now display products using card-based design with dynamic content from admin panel, replacing static product information. Main dashboard includes dedicated product information section showing all products configured in content management. Fixed defaultContent variable errors in admin panel. Dynamic product list synchronizes instantly between content management, order forms, and dashboard display sections. Wrapping cloths (보자기) fully integrated as regular products with unified management. Updated August 6, 2025.

**Dynamic Revenue Management with Product Integration**: Implemented comprehensive dynamic product integration in revenue management system. Sales management now displays product names dynamically from content management system instead of fixed "한과1호", "한과2호", "보자기" labels. Price settings system (product_0Price, product_1Price, product_0Cost, product_1Cost keys) fully integrated with revenue calculations for accurate cost and selling price tracking. Both desktop grid view and mobile list view updated to show dynamic product names. Order form pricing now reflects price settings in real-time. Fixed infinite loop issues in order form price loading. Updated August 6, 2025.

**Historical Order Data Priority in Cost Analysis**: Modified sales detail analysis to prioritize actual order-time product selections and pricing over current price settings for accurate cost analysis. Revenue analysis now uses order.smallBoxPrice, order.largeBoxPrice, and stored cost data as primary source, falling back to current settings only when historical data unavailable. Dynamic product quantities from JSON field integrated into cost calculations with proper product name display in both desktop and mobile views. This ensures cost analysis reflects real profit margins based on actual order conditions rather than current pricing configurations. Updated August 6, 2025.

**Flexible Shipping Cost Calculation System**: Implemented comprehensive shipping cost customization allowing admin to exclude wrapping cloth (보자기) from shipping quantity calculations. Added "excludeWrappingFromShipping" boolean setting to content management system with intuitive checkbox interface. Order form now dynamically adjusts shipping cost calculations based on this setting - when enabled, wrapping cloth quantities are excluded from free shipping threshold calculations while still being included in total order pricing. This provides operational flexibility for businesses that want to offer free shipping based on main product quantities only. Setting stored in dashboard content and synchronized across order forms in real-time. Updated August 6, 2025.
